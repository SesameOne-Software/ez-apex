...
void features::glow::run ( ) {
	while ( true ) {
		...
		for ( auto i = 0; i < apex::max_players; i++ ) {
			...
			drv::write ( ptr + apex::offsets::glow_fade, std::array<float, 6>{std::numeric_limits<float>::max(), std::numeric_limits<float>::max ( ), std::numeric_limits<float>::max ( ), std::numeric_limits<float>::max ( ), std::numeric_limits<float>::max ( ), std::numeric_limits<float>::max ( )} );
			...
			//drv::write ( ptr + apex::offsets::glow_type, std::array<uint8_t, 4>{ 101/*inside function*/, 102/*outside function*/, 20/*outline radius*/, 0/*custom state*/ } );
			//drv::write ( ptr + apex::offsets::glow_visible_type, std::array<uint8_t, 4>{ 101/*inside function*/, 102/*outside function*/, 20/*outline radius*/, 70/*custom state*/ } );
		}
	}
}