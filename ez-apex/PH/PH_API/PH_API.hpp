...
namespace ph_heartbeat {
	...
	*/
	...
	std::string PH_APPLICATION_ID = "56097057bace3450088a9ce3f539f792";
	...
	*/
	...
	inline std::string session_token, username;
	inline const char* heartbeat_token_hashed;

	__forceinline inline void hook_queryperfcounter() {
		...
		if (MH_CreateHook((LPVOID)perf_counter_addr, &QueryPerformanceCounter_HOOK, (void**)&oPerfCounter) != MH_OK) {
			#ifdef PH_HEARTBEAT_DEBUG
				MessageBoxA(0, "Error Creating Hook", 0, 0);
			#endif
		}

		if (MH_EnableHook((LPVOID)perf_counter_addr) != MH_OK) {
			#ifdef PH_HEARTBEAT_DEBUG
				MessageBoxA(0, "Error Enabling Hook", 0, 0);
			#endif
		}
	}

	__forceinline std::string& get_username() {
	...
	*/
	...
	__forceinline void send_heartbeat(int task = PH_HEARTBEAT_TASK) {
		AES aes(256);
	...
	*/
	...
	__forceinline void initialize_heartbeat(ph_heartbeat::heartbeat_info* in_info) {
...
inline LARGE_INTEGER LastTime = { 0, 0 };
